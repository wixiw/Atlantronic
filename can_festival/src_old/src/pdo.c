      ...
      if (Size && ((offset + Size) <= 64))
        {
          ...
          int resWLA = 0;
          resWLA = getODentry (d, index, subIndex, tmp, &ByteSize, &dataType, 0);
          if (resWLA !=               OD_SUCCESSFUL)
            {
              MSG_ERR (0x1013,
                       " Couldn't find mapped variable at index-subindex-size : ",
                       (UNS16) (*pMappingParameter));
              MSG_ERR (0x1013," index : ",index);
              MSG_ERR (0x1013," subindex : ",subIndex);
              MSG_ERR (0x1013," resWLA : ",resWLA);
              MSG_ERR (0x1013," ByteSize : ",ByteSize);
              MSG_ERR (0x1013," dataType : ",dataType);
              return 0xFF;
            }
