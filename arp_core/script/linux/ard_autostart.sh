...
if [ ! -x "$DAEMON" ] ; then
	cecho "red" "The autostart script is not present : $DAEMON, or it doesn't have execution rigths"
	exit 0
fi
...
do_start()
{
	...
	if [ -f /opt/conf/MATCH ]; then
		...
		start-stop-daemon -Smbv -x $DAEMON -p $PIDFILE \
			|| return 2
		cecho "yellow" "Autostarting binaries !"
		sleep 2
	else
		cecho "yellow" "/opt/conf/MATCH is not present, I am not autostarting binaries"
	fi
