...
{
    if( betweenMinusPi2AndPlusPi2(firstTurretAngle - secondTurretAngle) <= 1E-9 )
    {
        oIcrPosition.phi(firstTurretAngle);
        oIcrPosition.delta(0);
        cout << "angle equal" << endl;
        return;
    }
...
//    Vector2 icrDistances = invDet*(AInv*B);
    Vector2 icrDistances = (AInv*B);

    //TODO si le CIR est sous une tourelle faut prendre l'autre ...
    ICR firstIcr;
    ICR secondIcr;
    ...
    ICR candidate1 = firstIcr.transport(firstTurretPosition);
    ICR candidate2 = secondIcr.transport(secondTurretPosition);
...
//    oIcrPosition.delta((candidate1.delta()+candidate2.delta())/2);
    ...
    //TODO first and second candidate should be equal
}
