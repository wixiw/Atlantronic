...
BOOST_AUTO_TEST_CASE( Constructor_copy_DetectedObject )
{
    ...
    lsl::DetectedObject dobj;
    dobj.setScan(ls);

    lsl::DetectedCircle dcircle(dobj);

    BOOST_CHECK_CLOSE( dcircle.getApparentRange(), 0.67314560089181297, 1.f);
    BOOST_CHECK_CLOSE( dcircle.getApparentTheta(), -1.9513027039072615, 1.f);
    BOOST_CHECK_CLOSE( dcircle.getApparentTime(), 0.2, 1.f);
