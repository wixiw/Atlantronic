...
CSMNode::CSMNode()
{
  ...
  respawn_srv = nh.advertiseService("CSM/respawn", &CSMNode::respawnCallback, this);
}
                                      ...
                                      const Scan& scanMsg)
{
  ...
  try
  {
     tfListener_.lookupTransform (worldFrame_, baseFrame_, ros::Time(0), worldToBaseTf);
  }
  catch (tf::TransformException ex)
  {
  ...
  double x = worldToBaseTf.getOrigin().x();
  double y = worldToBaseTf.getOrigin().y();
  double th = tf::getYaw(worldToBaseTf.getRotation());
  ROS_WARN("transfrom : x=%f y=%f cap=%f",x,y,th );
...
bool CSMNode::respawnCallback(Init::Request& req, Init::Response& res)
{
	initialized_ = false;
	return true;
}
