        ...
        const double & iMaxVelocity, const double & iMaxAcceleration, const double & iMaxJerk, PosVelAcc & oNext)
{
    ...
    arp_ods::Log(DEBUG) << "          >>computeNextStep";
    ...
    if (ResultValue < 0)
        {
        arp_ods::Log(INFO) << "               ReflexxesAPI ERROR"<<ResultValue;
        if(ResultValue==ReflexxesAPI::RML_FINAL_STATE_REACHED) arp_ods::Log(INFO) << "               RML_FINAL_STATE_REACHED";
        if(ResultValue==ReflexxesAPI::RML_ERROR_INVALID_INPUT_VALUES) arp_ods::Log(INFO) << "               RML_ERROR_INVALID_INPUT_VALUES";
        if(ResultValue==ReflexxesAPI::RML_ERROR_EXECUTION_TIME_CALCULATION) arp_ods::Log(INFO) << "               RML_ERROR_EXECUTION_TIME_CALCULATION";
        if(ResultValue==ReflexxesAPI::RML_ERROR_SYNCHRONIZATION) arp_ods::Log(INFO) << "               RML_ERROR_SYNCHRONIZATION";
        if(ResultValue==ReflexxesAPI::RML_ERROR_NUMBER_OF_DOFS) arp_ods::Log(INFO) << "               RML_ERROR_NUMBER_OF_DOFS";
        if(ResultValue==ReflexxesAPI::RML_ERROR_NO_PHASE_SYNCHRONIZATION) arp_ods::Log(INFO) << "               RML_ERROR_NO_PHASE_SYNCHRONIZATION";
        if(ResultValue==ReflexxesAPI::RML_ERROR_NULL_POINTER) arp_ods::Log(INFO) << "               RML_ERROR_NULL_POINTER";
        if(ResultValue==ReflexxesAPI::RML_ERROR_EXECUTION_TIME_TOO_BIG) arp_ods::Log(INFO) << "               RML_ERROR_EXECUTION_TIME_TOO_BIG";
        //if(ResultValue==ReflexxesAPI::RML_ERROR_OVERRIDE_OUT_OF_RANGE) arp_ods::Log(INFO) << "               RML_ERROR_OVERRIDE_OUT_OF_RANGE";
